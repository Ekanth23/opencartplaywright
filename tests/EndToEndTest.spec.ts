...
async function performRegistration(page: Page): Promise<string> {
    ...
    await registrationPage.setLastName(RandomDataGenerator.getRandomLastName());

    let email: string = RandomDataGenerator.getRandomEmail();
    await registrationPage.setEmail(email);
    await registrationPage.setTelephone(RandomDataGenerator.getRandomPhoneNumber());
